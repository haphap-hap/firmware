...
void Hal::servo_init()
{
    ...
    _scs_bus.begin(UART_NUM_1, 1000000, 17, 18);

    auto motion =
        std::make_unique<Motion>(std::make_unique<ScsServo>(2, Vector2i(0, 900), "servo", "zero_pos_1", false),
                                 std::make_unique<ScsServo>(1, Vector2i(-1280, 1280), "servo", "zero_pos_2", true));
    motion->init();
